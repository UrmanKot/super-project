...
@UntilDestroy()
@Component({
  selector: 'pek-warehouse-delivery-request',
  templateUrl: './warehouse-delivery-request.component.html',
  styleUrls: ['./warehouse-delivery-request.component.scss']
})
export class WarehouseDeliveryRequestComponent implements OnInit, OnDestroy {
  ...
  materialsToQc() {
    this.modalService.confirm('success').subscribe(confirm => {
      if (confirm) {
        this.isLoading = false;
        this.requestsService.materialsToQc(+this.orderId).pipe(untilDestroyed(this)).subscribe(() => this.router.navigate(['../'], {relativeTo: this.route}));
      }
    });
  }
